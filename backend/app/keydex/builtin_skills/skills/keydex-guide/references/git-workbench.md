...
Keydex 的 Git 能力跟随当前加载的项目，只通过两个位置进入：顶部模式切换右侧的 Git 菜单，以及左侧搜索下方的系统级 Git 入口。顶部菜单适合更新、提交、推送、创建或切换分支；系统级入口会在主内容区打开 Git 面板，提供本地改动、提交历史、分支与远程、Stash、冲突和高级操作。两个入口读取并修改同一份项目 Git 状态。
...
1. 在 Git 面板的“本地改动”查看未跟踪、修改、删除、重命名和冲突文件。
2. 选中文件查看 Diff，可按文件、差异块或精确选中代码暂存/取消暂存。二进制、子模块、未完整加载或无法识别编码的内容只提供适用的文件级操作。
...
- 分支 Compare 同时展示双方独有提交、修订版本文件差异和当前工作区改动；可从修订版本文件树逐层打开任意文件。
- Compare 明确区分单提交、`A..B` 与 `A...B`，并展示实际 merge base。提交详情可直接进入全屏 Diff，也可按当前条件执行比较、Cherry-pick、Checkout、Revert 或经过影响预览的 Reset。
...
Git Diff 支持统一和并排视图、自动换行、并排同步滚动、上一个/下一个文件与差异，并会保存布局偏好。并排视图对齐旧行和新行，使用连接关系帮助追踪移动或替换；大文件采用虚拟渲染和按需加载。复制机器可用差异时选“复制原始补丁”，复制局部内容时选“复制选中代码”。

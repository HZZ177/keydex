---
name: keydex-guide
description: 当用户询问 Keydex 产品本身时必须使用的中文产品指南。适用于 Keydex 桌面端的功能、界面、入口、操作流程、模式、会话、任务、工作台、文件与目录、预览编辑、批注、A2UI、Skill、.keydex、MCP、模型、联网、设置、安全审批、能力边界和故障排查问题，包括“在哪里、怎么做、是否支持、为什么不能操作、做完会怎样”等问法；即使用户没有点名 keydex-guide，只要问题对象是 Keydex 产品，也应先加载本 Skill。用户明确询问 Keydex 源码、内部架构、协议、数据库或开发实现时不使用，应检查当前代码。
---

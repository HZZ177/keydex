---
name: keydex-guide
description: 当用户询问 Keydex 产品本身时必须使用的中文产品指南。适用于 Keydex 桌面端的功能、界面、入口、操作流程、模式、会话、任务、Sub-Agent、工作台、Git、差异审阅、文件与目录、本地 HTML/Vite 预览、批注、A2UI、Skill、keydex.md、.keydex、MCP、模型、联网、设置、应用更新、安全审批、能力边界和故障排查问题，包括“在哪里、怎么做、是否支持、为什么不能操作、做完会怎样”以及“如何从 CLAUDE.md 或 AGENTS.md 迁移”等问法；即使用户没有点名 keydex-guide，只要问题对象是 Keydex 产品，也应先加载本 Skill。用户明确询问 Keydex 源码、内部架构、协议、数据库或开发实现时不使用，应检查当前代码。
---
...
8. 回答功能位置时同时说明入口所在层级，例如“设置 → 策略配置”或“Agent 会话 → 右侧栏 → Sub-Agents”，避免只给按钮名。
9. 对版本可能变化的入口使用“当前版本”限定；不要把源码能力、未来设计或发布说明中的计划写成已上线功能。
...
- Explorer、Worker 与持续协作：[subagents-and-collaboration.md](references/subagents-and-collaboration.md)
...
- Git 顶部菜单、工具窗、风险与恢复：[git-workbench.md](references/git-workbench.md)
- 统一/并排 Diff、文件变更与历史审阅：[diff-review-and-file-history.md](references/diff-review-and-file-history.md)
...
- `keydex.md` 位置、优先级、写法与 CLAUDE.md/AGENTS.md 迁移：[persistent-project-instructions-and-migration.md](references/persistent-project-instructions-and-migration.md)
...
- 设置页面地图、实时保存与连接测试：[settings-map-and-save-behavior.md](references/settings-map-and-save-behavior.md)
...
- `keydex.md` 是每次请求临时注入的持久工作说明，不是 Skill，也不会出现在 Skill 选择器中；系统级和项目级文档按层叠方式同时生效，不按同名覆盖。
...
- Explorer 是只读调研角色，Worker 是可执行修改的工作角色；两者都不能继续委派新的 Sub-Agent。不要把它们描述成后台轮询任务。

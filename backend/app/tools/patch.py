...
支持的文件操作：
- *** Add File: <path>：创建新文件；正文每一行必须以 `+` 开头。
...
文件操作、`*** End Patch`。
文件操作层支持 `*** Add File: <path>`、`*** Update File: <path>` 和
`*** Delete File: <path>`。
Add File 正文每一行都以 `+` 开头；目标文件已存在时会失败。
...
def _parse_patch(patch: str) -> list[PatchOperation]:
    ...
    while index < len(lines) - 1:
        ...
        if header.startswith(UPDATE_FILE_MARKER):
            ...
            index = _collect_add_operation(lines, index, operations)
...
def _collect_add_operation(
    lines: list[str],
    index: int,
    operations: list[PatchOperation],
) -> int:
    path = _operation_layer_marker(lines[index]).removeprefix(ADD_FILE_MARKER).strip()
    if not path:
        raise ToolExecutionError("Add File 缺少路径", code="invalid_patch")
    index += 1
    ...
    if not body:
        raise ToolExecutionError(
            ...
            code="invalid_patch",
            details={
                "line": f"{ADD_FILE_MARKER}{path}",
                "line_number": body_start_line - 1,
                "hint": "Add File 正文至少需要一行以 + 开头的内容；空白行写成单独的 +。",
            },
        )
    operations.append(
        PatchOperation(kind="add", path=path, lines=body, body_start_line=body_start_line)
    )
    return index
...
def _preflight_changes(
    ...
    for operation in operations:
        ...
        if operation.kind == "add":
            planned_change = _plan_add(target, operation, context, state)
...
def _add_file_content(lines: list[str], *, path: str, start_line: int) -> str:
        ...
        if not line.startswith("+"):
            raise ToolExecutionError(
                ...
                details={
                    ...
                    "line_number": line_number,
                    "expected_prefixes": ["+"],
                    "hint": "Add File 中每一行新增内容都要写成 +<内容>；空白行写成单独的 +。",
                },
            )
...
def _change_result(change: PlannedChange, context: ToolExecutionContext) -> dict[str, Any]:
    ...
    operation = {
        ...
        "delete": "delete",
        "move": "move",
    }.get(change.kind, "update")
    change_type = "create" if change.kind == "add" else change.kind

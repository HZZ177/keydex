...
def build_a2ui_prompt_section(
    *,
    ...
    lines = [
        ...
        "当用户主动要求图表、趋势、分布、对比、漏斗、占比、确认、选择、补充参数或填写信息，且可用工具能匹配表达目标时，应优先考虑 A2UI。",
        ...
        "",
        "使用规则：",
        "- `chart` 用于自然呈现趋势、分布、对比、漏斗、横向指标条、占比等结构化数据，是只读渲染工具，不会阻塞执行；多个图表必须放在同一次调用的 `charts` 数组中，`summary` 只能是字符串。",

...
def build_a2ui_prompt_section(
    *,
    ...
    lines = [
        ...
        "- `choice`、`form` 是交互式工具，会等待用户提交或取消，然后用结构化结果恢复执行图；`choice.presentation_mode` 只是视觉呈现选择，`gallery` 与 `notification_stack` 功能接近，应在不影响表达清晰的前提下尽量交替使用，让两种组件都有展示机会，避免连续多次选择界面都长得一样，不要把两种模式当成严格场景分工。",

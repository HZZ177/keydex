...
def test_file_read_snapshot_store_records_and_isolates_contexts(tmp_path) -> None:
    ...
    assert read_current_file_content(target, context=first) == "hello\n"
...
def test_file_read_snapshot_store_does_not_downgrade_full_read_with_partial_read(tmp_path) -> None:
    ...
    assert read_current_file_content(target, context=context) == "hello\n"


def test_file_read_snapshot_store_discard_falls_back_to_current_file(tmp_path) -> None:
    target = tmp_path / "note.txt"
    target.write_text("hello\n", encoding="utf-8")
    context = _context(tmp_path, "first")
    ...
    assert read_current_file_content(target, context=context) == "updated\n"

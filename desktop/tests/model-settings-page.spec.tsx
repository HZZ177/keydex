...
describe("ModelSettingsPage", () => {
  it("renders provider cards with model and key state", async () => {
  ...
  it("renders empty state and create action", async () => {
    ...
    render(<ModelSettingsPage runtime={runtime} onCreateProvider={onCreateProvider} />);
  ...
  it("saves the default model from the settings page model list", async () => {
    ...
    const runtime = fakeRuntime(
      ...
      { setDefaultModel: vi.fn().mockResolvedValue(updated) },
    );

    render(<ModelSettingsPage runtime={runtime} />);

    fireEvent.click(await screen.findByRole("button", { name: "设为默认 deepseek-coder" }));

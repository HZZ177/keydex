...
describe("McpRuntime", () => {
  ...
  it("routes server, tool, prompt and runtime calls through MCP API paths", async () => {
    ...
    expect(request).toHaveBeenNthCalledWith(6, "/api/mcp/servers/srv%201/refresh", {
      method: "POST",
    });
    expect(request).toHaveBeenNthCalledWith(7, "/api/mcp/servers/refresh", {
      method: "POST",
    });
    expect(request).toHaveBeenNthCalledWith(
      8,
    ...
    expect(request).toHaveBeenNthCalledWith(10, "/api/mcp/servers/srv%201/tools/bulk-policy", {
      method: "POST",
  ...
  it("routes import, export, approval, oauth, trust and audit calls", async () => {
    ...
    expect(request).toHaveBeenNthCalledWith(3, "/api/mcp/approvals/approval%201/decision", {
      method: "POST",
      body: { decision: "approved", trust_scope: "server_readonly", user_id: "user-a" },
    });

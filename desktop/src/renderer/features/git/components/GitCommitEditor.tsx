...
export function GitCommitEditor({
  ...
  const stagedFiles = useMemo(
    () => status?.files.filter((file) => file.indexStatus !== null && !file.conflicted) ?? [],
    [status?.files],
  );
  ...
  const canCommit = validation.valid && identityReady && amendReady && (stagedFiles.length > 0 || amend || completesMerge) && !committing;
  ...
  return (
    <section className={styles.root} aria-label="Commit 编辑器">
      <header>
        ...
        <span>{stagedFiles.length} 个已暂存文件</span>
      </header>
      <textarea
        value={draft}
      ...
      <div className={styles.meta}>
        <span data-valid={validation.valid ? "true" : "false"}>{validation.message}</span>

...
export interface PreviewDiffViewProps {
  ...
  readonly onDisplayPreferenceChange?: (preference: { layout: KeydexDiffLayout; wrap: boolean }) => void;
}
...
export function PreviewDiffView({
  ...
  const setLayout = (layout: KeydexDiffLayout) => {
    ...
    onDisplayPreferenceChange?.({ layout, wrap: controller.state.wrap });
  };
  const setWrap = (nextWrap: boolean) => {
    controller.setWrap(nextWrap);
    displayPreference.update({ wrap: nextWrap });
    onDisplayPreferenceChange?.({ layout: controller.state.layout, wrap: nextWrap });
  };

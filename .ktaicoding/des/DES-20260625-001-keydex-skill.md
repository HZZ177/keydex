...
| 关联需求 | 对话需求：Keydex 内置/系统级/项目级 Skill 层级、`.keydex`、`/skill` 胶囊、`load_skill` 与预置底座 |
| 创建日期 | 2026-06-25 |
| 负责人 | Keydex |
| 状态 | 已实现，验收完成 |
| 最后更新 | 2026-07-15 |
...
- builtin/system/workspace 三层 Skill 底座，包括只读发布目录、固定系统根、manifest、scope 模型、Effective Snapshot、diagnostics、cache 与 watcher。
...
- system/workspace layer、Effective Snapshot、shadow barrier、三层 cache、watcher、统一 `keydexSkillsChanged`、Chat system-only 与项目继承/覆盖均已实现。
...
- source 优先级固定为 `workspace > system > builtin`；`inherit_system=false` 仅排除 system，builtin 始终作为最低层保留。
- 真实页面 E2E 使用独立临时 system/workspace 根和真实后端/Watcher，覆盖普通 Chat、项目继承/覆盖/关闭继承/barrier、历史恢复、只读预览和 workspace 隔离。
...
### 4.0 Builtin/System/Workspace 三层加载与层级控制

#### 4.0.1 功能目标与边界

Skill 不直接硬编码某一个 `.keydex/skills` 路径，而是消费统一的 layer profile、discovery 与 Effective Snapshot。Builtin、System 和 Workspace 都真实加载；普通 Chat 按 `system > builtin`、项目任务按 `workspace > system > builtin` 生成最终 winner。
...
app startup
  ...
  -> build builtin/system layer snapshot and system watcher

chat/project scope
  -> resolve optional workspace root
  -> build workspace layer snapshot
  -> resolve system-only or workspace-effective winner catalog with builtin base
...
系统与项目使用同构 `.keydex` 目录；builtin 是安装包内的只读发布数据。系统根第一期固定为 `Path.home() / ".keydex"`，不支持 `KEYDEX_SYSTEM_HOME`：
...
产品语义固定为三层；bundled preset Provisioner 仍只是 system 用户模板机制，不等同于 builtin：
...
```text
Keydex builtin release catalog
  < system .keydex
  < workspace .keydex
...
Keydex 在后端内存中分别固化 builtin/system/workspace 单层快照与最终有效快照。普通 Chat 的 system-only 模式实际解析 `system > builtin`；项目任务解析 `workspace > system > builtin`。prompt index、前端列表、`load_skill`、resource API、显式 `/skill` 和消息事件必须共享同一份 winner 视图。
...
builtin、system 与 workspace 分别发现后，由 Effective Catalog Resolver 应用 `inherit_system`、同名高层覆盖和高层无效候选 shadow barrier；所有消费者只获得 winner。普通 Chat 构建 builtin/system，项目任务再叠加 workspace。
...
产品展示三个来源 badge，但同名只展示一个有效 winner：
...
- workspace `skills.inherit_system` 默认 `true`；为 `false` 时只排除 system，项目仍消费 `workspace > builtin`。
- 有效 workspace 同名 Skill 覆盖 system/builtin，有效 system 同名 Skill 覆盖 builtin；UI、prompt、`load_skill` 和历史发送只看到 winner。
- workspace/system 中同名但无效的候选形成 shadow barrier，不回退更低层同名 Skill。
- system Manifest 无效只关闭 system Skill；合法 builtin 和 workspace Skill 仍可工作。
...
- [x] `backend/tests/keydex/test_system_profile.py` 与 `test_workspace_profile.py`：固定 system root、scope-aware manifest、默认值、非法 JSON、`skills.enabled=false`、`inherit_system`。
...
### 6.6 最终验收结果（2026-07-15）

- Keydex 后端完整影响面门禁：`262 passed`；覆盖 builtin catalog/hash、profile、resolver、snapshot/cache、watcher、API、Chat、prompt、activation、history、resource security、preset 与 sidecar 打包。
- Desktop Keydex 影响面门禁：`126 passed`；覆盖 runtime/hook、slash menu、SendBox、Home、Conversation、Preview、历史、错误恢复和 builtin“内置”来源展示。
- Desktop 类型检查：`tsc --noEmit` 通过。
- 真实 E2E：`keydex-system-skill.spec.ts` 与 `keydex-skill.spec.ts` 共 `2 passed`；使用独立真实后端、真实页面和临时 system/workspace 根，覆盖 builtin Chat 激活、三层继承/覆盖、`inherit_system=false` 保留 builtin、watcher、barrier、历史与只读预览。
...
- 契约清扫：产品代码无 `KEYDEX_SYSTEM_HOME`；builtin 不创建用户 `.keydex`；生产 bundled preset catalog 保持空数组，`keydex-guide` 仅存在于只读 builtin package。
...
| v1.3 | 2026-07-15 | 三层扩展 | 增加只读 builtin layer、发布 catalog/hash、`keydex-guide`、`workspace > system > builtin`、`inherit_system=false` 保留 builtin，以及 API/UI/history/package/E2E 合同 | 用户确认内置受发布方管理但允许 system/workspace 覆盖 |

---

本文档遵循 AICoding DES 范式。需求来源包括：2026-06-25 对话中关于 Keydex Skill、`load_skill` 与内部 force preset 的初始设计方向；2026-07-14 对话确认的固定系统根、普通 Chat 加载 system Skill、winner 覆盖式展示、系统资源只读访问和完整影响面测试；以及 2026-07-15 对话确认的只读 builtin 发布层、`keydex-guide`、`workspace > system > builtin` 与用户高层可覆盖语义。

...
- `.keydex` system/workspace 双层加载底座，包括固定系统根、manifest、scope 模型、Effective Snapshot、diagnostics、cache 与 watcher。
- system-only Chat 与 system + workspace 项目 Skill 发现接口，用于前端 slash menu 展示。
...
- 安全边界、失败处理、测试与验收方案。
- bundled preset catalog、一次性种子 Provisioner、用户所有权状态和 sidecar data 打包；当前 catalog 为空，不包含实际预置 Skill。

#### 本次明确不做

- 不支持从 `.agents/skills`、`.codex/skills`、`.claude` 自动迁移或兼容读取。
- 不支持用户可操作的 Skill 安装、更新、市场、版本管理或远程仓库拉取；bundled preset 只做应用内一次性种子初始化。
- 当前不预置任何真实、示例、占位或隐藏 Skill；空 catalog 不创建用户 `.keydex`。
...
- 双层有效视图：普通 Chat 使用 system-only，项目使用 system + workspace；同名时 workspace winner 覆盖 system，前端只显示 winner。
...
| 设计主题 | 结论 | 原因 |
|----------|------|------|
| Skill 来源 | 固定 system `.keydex/skills` 与项目根 workspace `.keydex/skills`，由后端输出 Effective Snapshot | 保持 `workspace > system` 的唯一真相，避免页面各自合并 |
| `.keydex` 加载前提 | 必须先实现统一 system/workspace loader 与 cache，Skill 只消费 loader 输出 | 避免 Skill 自己硬编码目录解析，保证 Chat、项目和 watcher 一致 |
| bundled preset | 实现一次性 System Layer 种子 Provisioner，当前 catalog 为空 | 为未来预置留完整能力，同时不增加第三运行时层或当前默认内容 |
...
|------|------|------|------|------|------|
| F-WS-01 | `.keydex` system/workspace Profile 与层级模型 | Codex 配置层按 home/project/cwd 组装有效配置；AGENTS.md 按 project root 到 cwd 分层加载。Keydex 借鉴“统一有效视图”，产品层级固定为 system 与 workspace 两层。 | Codex `codex-rs/config/src/loader/mod.rs:116`, `codex-rs/config/src/loader/mod.rs:1190`, `codex-rs/config/src/state.rs:483`, `codex-rs/core/src/agents_md.rs:167` | 启动/会话上下文 -> 加载启用的配置层 -> resolver 生成 effective config -> 后续能力只消费有效视图 | 新增/改造 `backend/app/keydex/profile.py`；system root 固定，workspace root 复用 `backend/app/services/workspace_service.py:40`, `backend/app/services/workspace_service.py:119` |
| F-WS-02 | Layer/Effective 快照、缓存与 watcher | Codex `SkillsManager` 按 cwd/config 缓存 `SkillLoadOutcome`，`skills/list` 支持 `force_reload`，文件 watcher 清 cache 并发 `SkillsChanged`。 | Codex `codex-rs/core-skills/src/manager.rs:52`, `codex-rs/core-skills/src/manager.rs:145`, `codex-rs/core-skills/src/manager.rs:196`, `codex-rs/app-server/src/request_processors/catalog_processor.rs:498`, `codex-rs/app-server/src/request_processors/catalog_processor.rs:563`, `codex-rs/app-server/src/skills_watcher.rs:145` | system/workspace roots -> layer snapshots -> effective winner -> 三层 memory cache -> watcher 精确失效与统一事件 | 新增/改造 `KeydexLayerRuntimeSnapshot`、`KeydexEffectiveRuntimeSnapshot`、`KeydexRuntimeCache` 与 system/workspace watcher |
...
| F-SK-11 | Skill 资源安全边界 | Codex 强制通过 `SKILL.md` 入口和 locator 描述资源来源；基座附录文件必须通过 `load_skill(resource_path=...)` 读取。Keydex 的两层 Skill 都是本地用户数据，必须限制在当前 winner 目录内，并设置路径、链接、大小和类型边界。 | Codex `codex-rs/core-skills/src/render.rs:25`; 基座 `agent_backend/tools/skill_activation_messages.py:15`, `common/common_service/skill_service.py:216`; Keydex `backend/app/api/workspace.py:812`, `backend/app/tools/filesystem.py:347`, `backend/app/tools/search.py:444`, `backend/app/tools/patch.py:520` | effective winner + resource path -> normalize/resolve -> must stay under winner dir -> link/size/type guard -> diagnostics/error | 新增 `backend/app/keydex/skills/security.py`；system 资源只走专用只读协议，任何 layer 都不得借 Skill 读取任意文件或执行脚本 |
...
### 4.0 `.keydex` System/Workspace 双层加载与层级控制

#### 4.0.1 功能目标与边界

Skill 不直接硬编码某一个 `.keydex/skills` 路径，而是消费统一的 layer profile、discovery 与 Effective Snapshot。System Layer 和 Workspace Layer 都真实加载；普通 Chat 只使用 system，项目任务按 `workspace > system` 生成最终 winner。
...
app startup
  ...
  -> build system layer snapshot/cache/watcher

chat/project scope
  -> resolve optional workspace root
  -> build workspace layer snapshot
  -> resolve system-only or workspace-effective winner catalog
  -> cache immutable Effective Snapshot
  -> prompt/UI/tool/resource/event share that snapshot
```

#### 4.0.2 `.keydex` 目录结构

系统与项目使用同构目录；系统根第一期固定为 `Path.home() / ".keydex"`，不支持 `KEYDEX_SYSTEM_HOME`：
...
两层 `keydex.json` 都是可选文件。目录不存在时返回正常空 layer；目录存在而 manifest 不存在时使用默认配置。
...
| `skills.enabled` | boolean | `true` | 是否启用当前 layer 的 Skill 发现 |
| `skills.inherit_system` | boolean | `true` | 仅 workspace 生效；是否把合法 system Skill 作为合并基底 |
...
| 层级 | 根目录 | 生效范围 | 优先级 |
|------|--------|----------|--------|
| `system` | `<user-home>/.keydex` | 普通 Chat 与默认项目继承 | 低 |
| `workspace` | `<workspace-root>/.keydex` | 当前项目 | 高 |
...
```python
KeydexScope = Literal["system", "workspace"]
...
class KeydexEffectiveProfile:
    ...
    workspace: KeydexLayerProfile | None
    diagnostics: tuple[KeydexDiagnostic, ...]
```

每层独立读取与发现，resolver 才执行继承、shadow barrier 和 winner 合并。system manifest 不消费 `inherit_system`；workspace 缺省为 `true`。`runtime_params` 只能从当前 Effective Snapshot 选择 winner，不参与配置合并。
...
flowchart TD
    ...
    B --> D["load system layer profile/catalog"]
    C -- "yes" --> E["load workspace layer profile/catalog"]
    C -- "no" --> F["system-only resolver"]
    D --> F
    D --> G["workspace-effective resolver"]
    E --> G
    G --> H{"workspace manifest valid?"}
    H -- "no" --> I["effective Skill subsystem unavailable + diagnostics"]
    H -- "yes" --> J["inherit system if enabled"]
    J --> K["apply workspace blocked_names shadow barrier"]
    K --> L["overlay valid workspace skills"]
    F --> M["immutable winner snapshot"]
    L --> M
```
...
```python
def load_layer(scope: KeydexScope, workspace_root: Path | None = None) -> KeydexLayerProfile:
    keydex_root = resolve_system_root() if scope == "system" else resolve_workspace_root(workspace_root) / ".keydex"
    manifest_path = keydex_root / "keydex.json"
    diagnostics = []

    manifest = {"schema_version": 1, "skills": {"enabled": True, "inherit_system": True}}
    if manifest_path.is_file():
        try:
            manifest = validate_manifest(scope, merge_defaults(manifest, read_json(manifest_path)))
        except KeydexManifestError as exc:
            return failed_layer(scope, keydex_root, exc)

    return KeydexLayerProfile(
        scope=scope,
        root=keydex_root,
        enabled=bool(manifest["skills"]["enabled"]),
        inherit_system=bool(manifest["skills"].get("inherit_system", True)),
        manifest=manifest,
        diagnostics=tuple(diagnostics),
    )

def build_effective(workspace_root: Path | None) -> KeydexEffectiveRuntimeSnapshot:
    system = discover_layer(load_layer("system"))
    if workspace_root is None:
        return resolve_system_only(system)
    workspace = discover_layer(load_layer("workspace", workspace_root))
    return resolve_workspace_effective(system, workspace)
```
...
- `backend/app/keydex/profile.py`、discovery、resolver、runtime cache 共同生成 Effective Snapshot。
- ChatService 按 session 类型获取 system-only 或 workspace-effective snapshot，并把同一对象写入 request/tool context。
- API、prompt、`load_skill`、resource 读取和历史事件禁止各自重新扫描/合并。

#### 4.0.8 边界与失败处理

| 场景 | 处理 |
|------|------|
| 任一 `.keydex` 不存在 | 该 layer 正常返回空 catalog，不自动创建目录 |
| 任一 `keydex.json` 不存在 | 使用对应 scope 默认 manifest |
| system manifest 无效 | system layer 失败关闭；普通 Chat 无 Skill 但仍可聊天；合法 workspace Skill仍可工作 |
| workspace manifest 无效 | 该项目 Effective Skill 子系统 unavailable；不回退成 system-only，不影响聊天与项目文件能力 |
| system `skills.enabled=false` | system layer 空，不影响 workspace Skill |
| workspace `skills.enabled=false` | 不扫描 workspace；若 `inherit_system=true` 仍返回 system winner |
...
Keydex 在后端内存中分别固化单层快照与最终有效快照。普通 Chat 使用 system-only Effective Snapshot；项目任务使用 system + workspace Effective Snapshot。prompt index、前端列表、`load_skill`、resource API、显式 `/skill` 和消息事件必须共享同一份 winner 视图。
...
fingerprint 不持久化到数据库，并覆盖影响执行结果的全部内容：规范化 layer root、`keydex.json`、Skill 目录结构、每个 `SKILL.md` 以及允许读取的附录文本资源。这样附录变化也能使下一轮获得新 snapshot，而运行中的 turn 仍保持旧 snapshot，不发生半轮切换。
...
| Home 选择项目 / 打开项目 session | 请求 workspace-effective Snapshot | slash menu 只展示覆盖后的 winner |
| 每个 turn 开始 | 从 cache 获取并固定一份 Effective Snapshot | prompt、preset、tool、event 使用同一 winner |
...
`KeydexRuntimeCache` 维护三类 cache：单例 system layer、按规范化 workspace root 分片的 workspace layer、按 `(system_fp, workspace_fp, inherit_system)` 派生的 effective snapshot。
...
- system 变化：失效 system cache 和全部依赖 system 的 effective cache。
- workspace 变化：只失效该 workspace layer 与对应 effective cache。
- `inherit_system=false`：该项目 effective key 不依赖 system fingerprint，system 变化不触发它刷新。
...
#### 4.0.12 双 Watcher 与统一前端事件

后端分别监听固定 system 根与活跃 workspace 根下的 `keydex.json`、`skills/**`。事件规范化、去抖后先精确 invalidate cache，再广播统一 `keydexSkillsChanged`；不再暴露 `workspaceSkillsChanged` 旧合同。

```mermaid
sequenceDiagram
    participant FS as "System/Workspace watcher"
    participant Cache as "KeydexRuntimeCache"
    participant WS as "WebSocket broadcaster"
    participant UI as "Desktop UI"
    participant API as "Effective Skills API"

    FS->>Cache: invalidate_system() / invalidate_workspace(root)
    FS->>WS: keydexSkillsChanged(scope, workspace_id?, revision)
    ...
    UI->>UI: atomically replace winner list
```
...
- `keydexSkillsChanged` 只刷新受影响的当前 scope；成功后整份原子替换。
...
system 与 workspace 分别发现后，由 Effective Catalog Resolver 应用 `inherit_system`、同名 workspace 覆盖和无效 workspace shadow barrier；所有消费者只获得 winner。普通 Chat 只构建 system layer，项目任务构建两层有效视图。
...
| 获取项目 Effective Skill | GET | `/api/workspaces/{workspace_id}/skills?force_reload=false` | Home / Desktop renderer | 在项目会话创建前返回 `workspace > system` winner |
...
本期不引入数据库持久缓存。后端使用进程内 `KeydexRuntimeCache` 管理 system layer、per-workspace layer 和 effective snapshot；前端使用 `EffectiveSkillState` 按 scope 原子替换 winner 列表。接口读取的不是临时拼接结果，而是对应 scope 的不可变运行时快照。
...
- 手动刷新 workspace tree。
- 收到后端 `keydexSkillsChanged` 通知且事件 scope 影响当前视图。
...
- 项目级：当前 workspace 自己的 Keydex 配置与 Skill。

普通 Chat 使用 system-only snapshot；项目任务使用 system + workspace Effective Snapshot。内部 source 固定为：

```ts
type SkillSource = "workspace" | "system";
```

#### 4.10.2 Manifest、覆盖与失败关闭

- `skills.enabled` 只控制当前 layer。
- workspace `skills.inherit_system` 默认 `true`；为 `false` 时项目只消费 workspace Skill。
- 有效 workspace 同名 Skill 覆盖 system；UI、prompt、`load_skill` 和历史发送只看到 winner。
- workspace 目录中同名但无效的候选形成 shadow barrier，不回退 system 同名 Skill。
- system Manifest 无效只关闭 system Skill，普通 Chat 仍可无 Skill 对话，合法 workspace Skill仍可工作。
- workspace Manifest 无效使当前项目 Skill 子系统失败关闭，不静默降级 system-only；项目其他聊天/文件能力仍可工作。

#### 4.10.3 Snapshot、cache 与 watcher

- system layer、per-workspace layer 和 Effective Snapshot 分别缓存。
- system 变化失效所有实际继承它的 effective cache；workspace 变化只影响该项目。
- fingerprint 覆盖 Manifest、`SKILL.md` 和全部普通资源文件。
- watcher 对 Chat 订阅 system，对项目订阅 workspace 并按 `inherit_system` 决定 system 依赖。
...
普通 Chat 可使用 system Skill index 与安全 `load_skill`，但不得因此注册 workspace tree/search/read/write、patch、shell、file history、annotation 或 workspace MCP 工具。system root 不加入 workspace roots；系统资源只能通过 Skill 专用只读协议加载或预览，不能通过通用文件 API 遍历。
...
预置模板未来只会被一次性种子复制到 system `.keydex/skills`，随后与用户自建系统 Skill 完全相同，继续遵守 `workspace > system`。
...
| 对象 | 所在层 | 变更 | 说明 |
|------|--------|------|------|
| `KeydexLayerProfile` | `backend/app/keydex/profile.py` | 修改/新增 | system/workspace 各自的 manifest、enabled、inherit 与 diagnostics |
| `KeydexLayerRuntimeSnapshot` | `backend/app/keydex/runtime.py` | 修改/新增 | 单层 profile、SkillLayerCatalog、fingerprint 与 loaded_at |
| `KeydexEffectiveRuntimeSnapshot` | `backend/app/keydex/runtime.py` | 新增 | system-only 或 workspace-effective 的不可变 winner 视图 |
| `KeydexRuntimeCache` | `backend/app/keydex/runtime_cache.py` | 修改 | system、per-workspace、effective 三层 cache 与依赖失效 |
...
| 前端 Skill state | 收到 keydexSkillsChanged | 正常 | 按受影响 scope 重新拉取并原子替换 slash menu winner |
| 历史回显 | 带 Skill 发送后刷新会话 | 正常 | 用户消息仍显示 Skill 胶囊 |

### 6.2 单元测试点

- [ ] `backend/tests/skills/test_frontmatter.py`：frontmatter 子集解析。
- [ ] `backend/tests/keydex/test_system_profile.py` 与 `test_workspace_profile.py`：固定 system root、scope-aware manifest、默认值、非法 JSON、`skills.enabled=false`、`inherit_system`。
- [ ] `backend/tests/keydex/test_system_runtime_cache.py` 与 `test_workspace_runtime_cache.py`：三层 snapshot/cache、fingerprint 命中、force reload、依赖 invalidate。
- [ ] `backend/tests/skills/test_discovery.py` 与 `test_effective_skill_catalog.py`：分层发现、同名 winner、shadow barrier、invalid diagnostics、稳定排序。
...
- [ ] `backend/tests/keydex/test_preset_provisioner.py`：空 catalog no-op、schema/hash、一次性种子、existing/edited/deleted/new id、state/lock/staging 和安全边界。
- [ ] `backend/tests/packaging/test_build_agent_server.py`：preset catalog/resources 进入 sidecar fingerprint 与 PyInstaller data。
...
- [ ] 应用启动严格执行 provision → system cache → watcher；当前空 catalog 不创建目录、不触发 watcher。
- [ ] 临时非空 bundle 种子后，普通 Chat 可加载该 system Skill，项目同名 Skill仍按 workspace winner 覆盖。

### 6.4 前端测试点

- [ ] `desktop/tests/slash-command-menu.spec.tsx`：动态 Skill command 过滤与选择。
- [ ] `desktop/tests/workspace-skill-state.spec.tsx`：system/workspace/session scope 加载、统一 changed 通知刷新、错误时保留旧列表。
...
- system/workspace 冲突只按 `workspace > system`；面板仅显示 winner，不提供同名版本选择。
- bundled preset 当前必须保持空 catalog；未来增加实际 preset 时只新增 bundle/catalog 内容，不修改双层 runtime 优先级。
...
本文档遵循 AICoding DES 范式。需求来源包括：2026-06-25 对话中关于 Keydex Skill、`load_skill` 与内部 force preset 的初始设计方向；以及 2026-07-14 对话确认的固定系统根、普通 Chat 加载 system Skill、项目双层覆盖、winner 覆盖式展示、系统资源只读访问、完整影响面测试和“预置能力先设计进合同、当前 catalog 为空、必须在 System Layer 后实施”的扩展要求。

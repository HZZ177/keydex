...
| F09 | Direct/Deferred Exposure | 支持工具多时通过 search/list 发现 | `exposure.py`、`search_mcp_tools` | exposure mode、search index | 是 |
...
| Direct/deferred exposure | `core/src/mcp_tool_exposure.rs:13-43`、`:51`、`:74` | 工具少直接暴露，工具多走 `search_mcp_tools`/`list_mcp_tools` |
...
| Lazy tool/search_tools | `agent_backend/tools/search_tools.py:18-54`、`lazy_tool_middleware.py:38-103`、`assembler.py:1341-1347` | 借鉴大工具集 deferred exposure 的用户可搜索工具机制 |
...
4. 被全局禁用、server 禁用、会话禁用、hidden、removed 或离线剔除的 tool 不进入 direct exposure、deferred search/list，也不进入 snapshot 的 `visible_tools_json`；因此它的 `model_name`、`description`、`input_schema` 都不会出现在下一次 Agent tool 注入内容中。
...
### 4.9 F09 Direct/Deferred Tool Exposure

#### 4.9.1 功能目标与边界

当 MCP tools 数量较少时直接暴露给模型；当工具数量大或用户开启延迟暴露时，只直接暴露 `search_mcp_tools` 与 `list_mcp_tools`，模型按需查询后在下一 step/下一轮获得目标工具。

#### 4.9.2 策略

| 模式 | 触发 | 暴露给模型 |
|------|------|------------|
| direct | 可见 MCP tools 数量小于阈值，或用户强制 direct | 全部可见 MCP tools |
| deferred | tools 数量超过阈值，或 server/tool policy 指定 | `search_mcp_tools`、`list_mcp_tools`、已激活 tools |
| selected direct | `allow_selected_only` 且用户已选择少量 tools | 已选择 tools |

#### 4.9.3 search_mcp_tools 逻辑

- 输入：关键词、server filter、risk filter、limit。
- 数据来源：snapshot 中 deferred tools 的描述和 schema 摘要。
- 输出：tool model name、server、description、risk、activation hint。
- 激活：会话级 active window，参考 kt-agent lazy tool active window，但 Keydex 需与 snapshot 语义一致。若当前模型请求已发出，只返回建议；下一轮或下一 step 注入。
...
- 会话级禁用只影响当前会话；它会从后续 snapshot 和 deferred search/list 中移除该 tool 的 `model_name`、`description`、`input_schema`，审批策略仍保持全局配置不变。
...
| `exposure.py` | direct/deferred/allow/deny/model-visible |
| `tools.py` | `McpLocalTool` 和 search/list tools |
...
| `KEYDEX_MCP_DEFERRED_TOOL_THRESHOLD` | `40` | 超过阈值启用 deferred exposure |
...
| F09 Deferred exposure | search index、active window、threshold 判断 | 超阈值只暴露 search/list；搜索后返回目标 tools | 大 server 只显示 search_mcp_tools，搜索后提示下一轮可用 |
...
| 大 MCP server 工具过多 | prompt/tool schema 过大 | deferred exposure、search_mcp_tools、threshold |

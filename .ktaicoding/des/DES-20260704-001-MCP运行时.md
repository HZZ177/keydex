...
| F09 | 直接注入预算与能力目录 | 支持工具多时通过统一能力发现入口按需激活 | `exposure.py`、`discover_mcp_tools` | direct budget、capability directory、active window | 是 |
...
当 MCP tools 数量较少时全部直接暴露给模型；当工具数量超过直接注入预算时，模型仍能看到一个统一的 MCP 能力发现入口 `discover_mcp_tools`。该入口无 query 时返回服务器级能力目录，有 query 时返回匹配工具并激活命中项；激活后的工具在同一 agent 执行链路的后续模型调用中成为直接可用工具。

#### 4.9.2 策略

| 模式 | 触发 | 暴露给模型 |
|------|------|------------|
| 直接可用 | 可见 MCP tools 数量小于等于直接注入预算，或超过预算后工具已被发现入口命中激活；激活后直接可用数量仍受预算上限约束 | 对应 MCP tools |
| 按需加载 | tools 数量超过直接注入预算时默认全部进入能力目录；已激活工具从目录中临时提升为直接可用，其余继续留在目录 | `discover_mcp_tools` 能力发现入口和服务器能力目录 |
...
| `KEYDEX_MCP_DIRECT_TOOL_BUDGET` | `10` | 每轮直接注入的 MCP 工具预算，超出部分进入能力目录按需加载 |
...
| F09 直接注入预算与能力目录 | search index、active window、直接注入预算判断 | 大工具集暴露 `discover_mcp_tools` 和服务器能力目录；搜索后激活目标 tools | 59 工具 server 显示能力目录，发现入口激活后目标工具可调用 |

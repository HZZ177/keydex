...
Keydex 没有直接复刻 Claude Code 的大小写工具名，而是使用既有 snake_case 工具名。`edit_file(old_string="")` 对齐 Claude Code 的创建/空文件写入语义，但普通新文件创建仍优先使用 `create_file`。
...
- `edit_file`、`delete_file`、`move_file` 都可直接操作已有文件，不要求预先完整 `read_file`。
- `edit_file` 的 `old_string` 非空时必须精确匹配，且默认只能匹配一次。
- `edit_file` 的 `old_string` 为空时仅用于创建不存在文件，或写入已有空文件/纯空白文件；目标非空会失败。
- 多处匹配必须设置 `replace_all=true`。
- 如果此前读取过文件，且读取后文件发生变化，会拒绝覆盖当前内容并提示重新读取。
